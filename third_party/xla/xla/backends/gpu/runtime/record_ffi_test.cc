...
absl::StatusOr<void*> GetNativeFunctionPtr(const se::Kernel* kernel);
...
TEST(RecordFfiTest, KernelLaunchWithCuFunc) {
  ...
  ASSERT_OK_AND_ASSIGN(
      std::unique_ptr<se::Kernel> loaded_kernel,
      executor->LoadKernel(se::KernelLoaderSpec::CreateCudaCubinInMemorySpec(
          binary.bytes, "AddI32", /*arity=*/3)));
  absl::StatusOr<void*> func_ptr = GetNativeFunctionPtr(loaded_kernel.get());
