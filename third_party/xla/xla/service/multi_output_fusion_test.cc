...
// Fuses elementwise siblings and records how often the sibling worklist build
...
class CountingMultiOutputFusion : public MultiOutputFusion {
 public:
  ...
  absl::flat_hash_map<std::string, int> worklist_is_fusible_calls;
  absl::flat_hash_map<std::pair<std::string, std::string>, int>
      worklist_legal_to_fuse_calls;
 ...
 protected:
  ...
  bool IsFusible(HloInstruction* instr) override {
    ++is_fusible_calls_[std::string(instr->name())];
  ...
  void CreateFusionWorkListForCurrentComputation() override {
    ...
    worklist_is_fusible_calls = is_fusible_calls_;
    worklist_legal_to_fuse_calls = legal_to_fuse_calls_;
  }
...
ENTRY e {
  ...
  EXPECT_THAT(fusion.worklist_is_fusible_calls,
              UnorderedElementsAre(Pair("p0", 1), Pair("p1", 1), Pair("a", 1),
                                   Pair("b", 1), Pair("c", 1), Pair("t", 1)));
  EXPECT_THAT(
      fusion.worklist_legal_to_fuse_calls,
