...
SpmdPartitioningVisitor::ManualToOneDeviceSharding(HloOpcode opcode,
  ...
  if (!sharding.IsTuple()) {
    ...
    return std::make_shared<const HloSharding>(HloSharding::SingleDevice(0));
  }
  ...
  auto it = manual_to_one_device_shardings_.find(key);
  if (it != manual_to_one_device_shardings_.end()) {
    return it->second;
  }
  ...
  manual_to_one_device_shardings_.emplace(std::move(key), result);
  return result;
}
