    ...
    for (HloInstruction* instruction :
      ...
      switch (instruction->opcode()) {
        ...
        case HloOpcode::kAsyncUpdate: {
          ...
          // contexts}. It defines all of the tuple-shaped values and the
          // contexts. When the async thread is excluded (or control-flow
          // propagation is disabled) and the output at {1, ...} is late-bound,
          // AsyncUpdate also defines the newly bound output values.
          bool thread_included =
              propagate_through_control_flow_ &&
              HloInstruction::IsThreadIncluded(
                  instruction->async_execution_thread(), execution_threads_);
        ...
        case HloOpcode::kAsyncDone: {
          ...
          // tuple-shaped values, plus any late-bound output values when the
          // async thread is excluded (or control-flow propagation is disabled).
          bool thread_included =
              propagate_through_control_flow_ &&
              HloInstruction::IsThreadIncluded(
                  instruction->async_execution_thread(), execution_threads_);
